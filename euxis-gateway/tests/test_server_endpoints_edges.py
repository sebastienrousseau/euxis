...
def test_telegram_webhook_direct(monkeypatch):
    ...
    scope = {"type": "http", "headers": [(b"x-telegram-bot-api-secret-token", b"secret")], "query_string": b"", "client": ("127.0.0.1", 1234)}
    req = Request(scope)
...
def test_voice_upload_branches(monkeypatch):
    ...
    scope = {"type": "http", "headers": [(b"authorization", b"Bearer tok")], "query_string": b"", "client": ("127.0.0.1", 1234)}
    req = Request(scope)
