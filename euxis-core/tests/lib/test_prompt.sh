...
if [[ -f "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_security-boundaries.txt" ]]; then
    sec_content=$(head -5 "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_security-boundaries.txt")
...
if [[ -f "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_graph-memory.txt" ]]; then
