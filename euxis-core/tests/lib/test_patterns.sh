...
for p in $EXPECTED_PATTERNS; do
    assert_file_exists "pattern ${p} exists" "${EUXIS_HOME}/euxis-core/config/patterns/${p}"
done

# Test: README exists
assert_file_exists "patterns README exists" "${EUXIS_HOME}/euxis-core/config/patterns/README.md"

# Test: each pattern has required sections
for p in $EXPECTED_PATTERNS; do
    pf="${EUXIS_HOME}/euxis-core/config/patterns/${p}"
    [[ -f "${pf}" ]] || continue
    name="${p%.md}"
...
for f in "${EUXIS_HOME}/euxis-core/config/patterns/"*.md; do
    [[ -f "$f" ]] || continue
    name=$(basename "$f")
...
count=$(ls -1 "${EUXIS_HOME}/euxis-core/config/patterns/"*.md 2>/dev/null | grep -v README | wc -l | tr -d ' ')
