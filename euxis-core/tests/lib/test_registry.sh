...
registry="${EUXIS_HOME}/euxis-core/agents/registry.json"
...
# All versions are v0.0.3
non_007=$(jq '[.agents[] | select(.version != "v0.0.3")] | length' "${registry}" 2>/dev/null)
assert_eq "all versions v0.0.3" "0" "${non_007}"
