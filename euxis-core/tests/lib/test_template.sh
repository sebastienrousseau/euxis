...
assert_contains "template replaces PROJECTS_DIR" "${EUXIS_HOME}/euxis-runtime/data/projects" "${result}"
...
assert_eq "no sed in template.sh" "0" "$(grep -c '| sed' "${EUXIS_HOME}/euxis-core/lib/template.sh")"
