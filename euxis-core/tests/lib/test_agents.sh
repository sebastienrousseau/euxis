...
assert_eq "resolve architect path" "${EUXIS_HOME}/euxis-core/agents/prompts/core/architect.txt" "${result}"

# Test resolve_agent_path: known fleet agent
result=$(resolve_agent_path "bug-fixer")
assert_eq "resolve bug-fixer path" "${EUXIS_HOME}/euxis-core/agents/prompts/fleet/bug-fixer.txt" "${result}"
