...
setup() {
    ...
    cat > "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_common.txt" << 'EOF'
# Common Protocol
All agents must follow these guidelines.
EOF

    cat > "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_protocol.txt" << 'EOF'
# Base Protocol
Standard operating procedures.
EOF

    cat > "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_security-boundaries.txt" << 'EOF'
# Security Protocol
Handle secrets and credentials carefully.
EOF

    cat > "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_versioning.txt" << 'EOF'
# Versioning Protocol
Follow semver for releases.
EOF

    # Create mock agent files
    cat > "${EUXIS_HOME}/euxis-core/agents/prompts/core/architect.txt" << 'EOF'
---
agent_id: architect
...
EOF
    ...
    cat > "${EUXIS_HOME}/euxis-runtime/data/projects/test/architect/memory.md" << 'EOF'
# Memory: architect
...
@test "resolve_protocols respects token budget" {
    ...
    local large_file="${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_security-boundaries.txt"
...
@test "resolve_protocols handles missing protocol files" {
    rm -f "${EUXIS_HOME}/euxis-core/agents/prompts/protocols/_security-boundaries.txt"
...
@test "prepare_prompt assembles full prompt" {
    local audit_path="${EUXIS_HOME}/euxis-runtime/data/projects/test/architect/audit.md"
    local memory_path="${EUXIS_HOME}/euxis-runtime/data/projects/test/architect/memory.md"
    touch "${audit_path}"
...
@test "resolve_protocols handles missing protocols directory" {
    rm -rf "${EUXIS_HOME}/euxis-core/agents/prompts/protocols"
