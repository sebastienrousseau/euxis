...
bool FileTree::handle_event(const Event& event) {
    ...
    auto clamp_scroll = [this]() {
        if (selected_ < scroll_) scroll_ = selected_;
        if (visible_rows_ > 0 && selected_ >= scroll_ + visible_rows_)
...
void Workspace::render(terminal::TerminalScreen& screen, Rect area) {
    ...
    // Separator color: use color system if available, else default muted
    uint8_t sep_r = 68, sep_g = 75, sep_b = 106;
    if (color_system_) {
        auto c = color_system_->color(SemanticRole::Muted);
        sep_r = c.r; sep_g = c.g; sep_b = c.b;
    }
    ...
    switch (mode_) {
        ...
        case LayoutMode::DualPane: {
            ...
            if (sidebar_visible_ && sidebar_) {
                ...
                for (int y = area.y; y < area.y + area.h; ++y) {
                    screen.set_cell(area.x + chat_w, y, U'│', sep_r, sep_g, sep_b);
                }
        ...
        case LayoutMode::TriplePane: {
            ...
            if (file_tree_) {
                ...
                // Separator
                for (int y = area.y; y < area.y + area.h; ++y) {
                    screen.set_cell(area.x + tree_w - 1, y, U'│', sep_r, sep_g, sep_b);
                }
            }
            ...
            if (sidebar_visible_ && sidebar_) {
                ...
                for (int y = area.y; y < area.y + area.h; ++y) {
                    screen.set_cell(sep_x, y, U'│', sep_r, sep_g, sep_b);
                }
...
bool Workspace::handle_event(const Event& event) {
    ...
    if (sidebar_visible_ && sidebar_ && sidebar_->handle_event(event)) return true;
    if (mode_ == LayoutMode::TriplePane && file_tree_ && file_tree_->handle_event(event)) return true;
    return false;
}
