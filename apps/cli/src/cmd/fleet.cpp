...
} // namespace (close anonymous namespace for externally-visible policy types)
...
int cmd_playbook(Context& ctx, const std::vector<std::string>& args) {
    ...
    if (compare_mode) {
        ...
        for (size_t i = 0; i < args.size(); ++i) {
            ...
            if (args[i] == "--ci") { base_args.push_back(args[i]); continue; }
            if (args[i] == "--policy") {
                base_args.push_back(args[i]);
                if (i + 1 < args.size()) base_args.push_back(args[++i]);
                continue;
            }
    ...
    // Incorporate builtin & claim-verifier failures into verdict math.
    // These entries have raw_verdict="FAILED" (not "FAIL") and are not in step_verdicts.
    int builtin_fails = 0;
    bool build_verification_failed = false;
    for (const auto& ev : evidence_log) {
        bool is_infra = (ev.agent_id.find("builtin-") == 0 ||
                         ev.agent_id.find("claim-verifier") == 0);
        if (!is_infra) continue;
        if (ev.verdict == "FAILED") {
            builtin_fails++;
            raw_fail_count++;
            if (ev.pillar == "build") build_verification_failed = true;
        }
    }
    // Include decisive negatives AND builtin failures in the effective fail count
    int effective_fails = fails + decisive_negatives + builtin_fails;
    ...
    // Factor: Builtin/claim-verifier failures (weight: 15 per, build=20)
    if (builtin_fails > 0) {
        int builtin_penalty = build_verification_failed ? 20 : (builtin_fails * 15);
        confidence -= builtin_penalty;
        std::string bf_detail = std::to_string(builtin_fails) + " builtin check(s) failed";
        if (build_verification_failed) bf_detail += ", build verification failed";
        confidence_factors.push_back("Builtin failures: -" + std::to_string(builtin_penalty) + " (" + bf_detail + ")");
    }
    ...
    if (has_contradiction) {
        ...
        if (contradictions.empty()) {
            ...
            for (const auto& ev : evidence_log) {
                ...
                if (ev.verdict == "FAILED" && fail_agent.empty()) fail_agent = ev.agent_name;
            }
            contradictions.push_back({"cross-pillar", pass_agent, "PASS", fail_agent, "FAILED",
    ...
    if (verdict_text == "TRUSTED") {
        ...
        if (build_verification_failed) recommended_action += " Build verification failed — fix build before merging.";
        else if (builtin_fails > 0) recommended_action += " " + std::to_string(builtin_fails) + " builtin check(s) failed.";
        ...
        recommended_action = "Fix blocking issues before merge.";
        if (build_verification_failed) recommended_action += " Build verification failed.";
        else recommended_action += " All agents agree on failure.";
    } else {
    ...
    // --- P1-7: Machine-Readable Provenance (SLSA v1) ---
    {
        ...
        provenance["builder"] = {{"id", "euxis-cli/v0.1.3"}};
