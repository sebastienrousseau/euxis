...
int cmd_policy(Context& ctx, const std::vector<std::string>& args) {
    ...
    if (sub == "check") {
        ...
        auto policy = load_policy(ctx.data_dir);
        if (!policy) {
            std::cerr << tr("No policy file found.") << "\n";
            return 1;
        }

        auto violations = evaluate_policy(artifact, *policy);

        std::cout << term::bold(tr("Policy Check")) << " (" << tr("latest artifact") << ")\n";

        // Show all gates with pass/fail status
        auto show_gate = [&](const std::string& gate, bool failed) {
            if (failed) {
        ...
        // Check each configured gate
        static const std::vector<std::string> gate_names = {
            "min_verdict", "min_confidence", "min_critical_coverage",
            "fail_on_regression", "max_drift", "fail_on_escalation",
            "fail_on_budget_exceeded"
        };
        for (const auto& gate : gate_names) {
            ...
            for (const auto& v : violations) {
                if (v.gate == gate) { violated = true; break; }
            }
            show_gate(gate, violated);
        }

        std::cout << "\n";
        if (!violations.empty()) {
            std::cout << term::red(std::to_string(violations.size()) + " policy violation(s)") << "\n";
            for (const auto& v : violations) {
                std::cout << "  " << v.gate << ": " << v.detail << "\n";
            }
            return 1;
        }
