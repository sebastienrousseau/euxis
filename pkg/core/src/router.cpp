...
void FinOpsRouter::track_session_usage(const std::string& session_id,
    ...
    // Manage session tracking with LRU-like eviction
    if (!session_usage_.contains(session_id)) {
        session_order_.push_back(session_id);
        enforce_limits();
    } else {
        // Move to back to mark as recently used
...
void FinOpsRouter::enforce_limits() {
    while (session_order_.size() > session_limit_) {
        const std::string victim = session_order_.front();
        session_order_.pop_front();
        session_usage_.erase(victim);
        spdlog::warn("Evicting old session telemetry for {} to maintain memory limits", victim);
    }
}
