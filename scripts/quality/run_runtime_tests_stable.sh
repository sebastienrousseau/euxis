...
if [[ ! -x "${TIMEOUT_BIN}" ]]; then
  echo "missing /usr/bin/timeout; cannot enforce bounded runtime test runtime" >&2
  exit 1
fi
...
done < <(find euxis-runtime/tests -maxdepth 2 -type f -name 'test_*.py' | sort)

if [[ ${status} -eq 0 ]]; then
  echo "== euxis-runtime/tests (coverage gate) =="
  pushd euxis-runtime >/dev/null
  PYTEST_LOCAL="${PYTEST}"
  ...
  if [[ ${gate_rc} -ne 0 ]]; then
    status=${gate_rc}
    echo "FAILED: euxis-runtime consolidated coverage gate (exit ${status})" >&2
  fi
fi
