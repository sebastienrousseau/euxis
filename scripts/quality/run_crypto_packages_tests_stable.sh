...
echo "== euxis-crypto-packages (workspace tests) =="
"${TIMEOUT_BIN}" -k 30 240 npm --prefix euxis-crypto-packages test

echo "== euxis-crypto-packages/src/crypto-lib (coverage gate) =="
"${TIMEOUT_BIN}" -k 30 360 npm --prefix euxis-crypto-packages/src/crypto-lib run test:ci

echo "== euxis-crypto-packages/src/crypto-lib (critical coverage thresholds) =="
node <<'NODE'
const fs = require('fs');

const coveragePath = 'euxis-crypto-packages/src/crypto-lib/coverage/coverage-final.json';
