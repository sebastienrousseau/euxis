## Package Benchmark Regression Validation

Threshold: `20.00%`
Mode: `blocking`

| Package | Wall p95 Δ | CPU ms p95 Δ | Memory p95 Δ | Status |
| --- | ---: | ---: | ---: | --- |
| `euxis-adapters` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-cli` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-core` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-crypto` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-web` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-docs` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-gateway` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-metrics` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-runtime` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-scripts` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-sdk` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-security` | +0.00% | +0.00% | +0.00% | ok |
| `euxis-tui` | +0.00% | +0.00% | +0.00% | ok |

Regressed packages: `0`
