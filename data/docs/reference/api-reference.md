...
| `_perf_record` | `_perf_record "op" "$ms" "agent" "ok"` | Append JSONL metric to `$EUXIS_HOME/euxis-data/runtime/perf/metrics.jsonl` |
...
- `EUXIS_PERF_LOG` — Path to JSONL performance metrics file (default: `$EUXIS_HOME/euxis-data/runtime/perf/metrics.jsonl`)
