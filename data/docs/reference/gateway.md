...
```bash
python3 api/src/gateway/protocol_test.py
```
...
Webhook updates are persisted to `~/.euxis/euxis-data/runtime/gateway/webhooks.json`.
...
```bash
python3 api/src/gateway/protocol_test.py --frames /path/to/frames.jsonl
```
