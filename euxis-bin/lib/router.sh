...
#
# Usage:
#   source "${EUXIS_HOME}/euxis-bin/lib/router.sh"
...
ROUTER_REGISTRY="${EUXIS_HOME}/euxis-data/agents/registry.json"
ROUTER_CONFIG="${EUXIS_HOME}/euxis-data/config/router.json"
...
router_should_warmup() {
    local cache_file="${EUXIS_HOME}/euxis-data/runtime/cache/.last_warmup"
...
router_record_warmup() {
    local cache_dir="${EUXIS_HOME}/euxis-data/runtime/cache"
    mkdir -p "$cache_dir"
