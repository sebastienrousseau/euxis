...
REGISTRY_DB="${EUXIS_HOME}/euxis-data/agents/registry.db"

# Connection pool directory for named pipes
REGISTRY_POOL_DIR="${EUXIS_HOME}/euxis-data/runtime/registry_pool"
...
resolve_agent_path_sql() {
    ...
    # - euxis-data/agents/prompts/core/foo.txt (modern)
    ...
    if [[ "${path}" == prompts/* ]]; then
        candidates+=("euxis-data/agents/${path}")
    fi
    if [[ "${path}" == agents/* ]]; then
        candidates+=("euxis-data/${path}")
    fi
...
registry_health_sql() {
    ...
    file_count=$(find "${EUXIS_HOME}/euxis-data/agents/prompts/core" "${EUXIS_HOME}/euxis-data/agents/prompts/fleet" -name "*.txt" 2>/dev/null | grep -v "/_" | wc -l | tr -d ' ')
...
_registry_auto_rebuild() {
    ...
    if [[ "${EUXIS_HOME}/euxis-data/agents/registry.json" -nt "${REGISTRY_DB}" ]]; then
...
registry_get_version() {
    ...
    if [[ -f "${EUXIS_HOME}/euxis-data/agents/registry.json" ]] && command -v jq &>/dev/null; then
        jq -r '.protocol_version // .version' "${EUXIS_HOME}/euxis-data/agents/registry.json" 2>/dev/null && return
    fi
    echo "v0.0.3"
}
...
registry_agent_exists() {
    ...
    if [[ -f "${EUXIS_HOME}/euxis-data/agents/registry.json" ]] && command -v jq &>/dev/null; then
        jq -e --arg id "${agent}" '.agents[] | select(.id == $id)' "${EUXIS_HOME}/euxis-data/agents/registry.json" &>/dev/null && return 0
    fi
    return 1
}
...
registry_list_agent_ids() {
    ...
    if [[ -f "${EUXIS_HOME}/euxis-data/agents/registry.json" ]] && command -v jq &>/dev/null; then
        jq -r '.agents[].id' "${EUXIS_HOME}/euxis-data/agents/registry.json" 2>/dev/null && return
    fi
}
