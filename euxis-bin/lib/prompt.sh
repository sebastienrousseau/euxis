...
_get_fleet_roster() {
    ...
    if [[ -n "${_EUXIS_REGISTRY_CACHE}" ]]; then
        ...
        local registry_file="${EUXIS_HOME}/euxis-data/agents/registry.db"
        [[ -f "${registry_file}" ]] || registry_file="${EUXIS_HOME}/euxis-data/agents/registry.json"
    ...
    # Try SQLite first
    local db_file="${EUXIS_HOME}/euxis-data/agents/registry.db"
    if [[ -f "${db_file}" ]]; then
        local ids
    ...
    # Fall back to JSON
    local registry_file="${EUXIS_HOME}/euxis-data/agents/registry.json"
