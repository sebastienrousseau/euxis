...
if [[ -f "${EUXIS_HOME}/euxis-bin/lib/registry_sql.sh" ]]; then
    source "${EUXIS_HOME}/euxis-bin/lib/registry_sql.sh"
...
EUXIS_LIFECYCLE_DIR="${EUXIS_HOME}/euxis-data/runtime/lifecycle"
...
EUXIS_PLUGINS_DIR="${EUXIS_HOME}/euxis-data/config/plugins"
...
agent_probe_readiness() {
    ...
    source "${EUXIS_HOME}/euxis-bin/lib/providers.sh" 2>/dev/null || true
    local provider
