...
COMMON_SH="${EUXIS_HOME}/euxis-bin/lib/common.sh"
if [[ -f "${COMMON_SH}" ]]; then
  # shellcheck source=core/lib/common.sh
  source "${COMMON_SH}"
else
  # shellcheck source=core/lib/common.sh
  source "${REPO_ROOT}/euxis-bin/lib/common.sh"
fi
...
ROUTER_SH="${EUXIS_HOME}/euxis-bin/lib/router.sh"
if [[ -f "$ROUTER_SH" ]]; then
    # shellcheck source=euxis-bin/lib/router.sh
    source "$ROUTER_SH"
...
# ============================================================================
# Provider Fallback Chain (v0.0.3)
...
EUXIS_PROVIDER_USAGE_DIR="${EUXIS_PROVIDER_USAGE_DIR:-${EUXIS_HOME}/euxis-data/runtime/provider-usage}"
...
EUXIS_CONTEXT_COMPACTION_CMD="${EUXIS_CONTEXT_COMPACTION_CMD:-${EUXIS_HOME}/euxis-bin/euxis-context-worker}"
...
run_with_fallback() {
    ...
    if [[ -f "${EUXIS_HOME}/euxis-bin/lib/core.sh" ]]; then
        source "${EUXIS_HOME}/euxis-bin/lib/core.sh" 2>/dev/null || true
    fi
