...
validate_euxis_structure() {
    ...
    local required_dirs=(
        "$euxis_home"
        "$euxis_home/euxis-bin"
        "$euxis_home/euxis-bin/lib"
        "$euxis_home/euxis-data/agents/prompts"
        "$euxis_home/euxis-data/runtime/memory/cortex"
        "$euxis_home/data"
    )
    ...
    if [[ -f "$euxis_home/euxis-data/agents/registry.db" ]]; then
        # Validate SQLite database
        local agent_count
        # SECURITY: Use sqlite3 CLI with proper path quoting to prevent injection
        agent_count=$(sqlite3 -init /dev/null "${euxis_home}/euxis-data/agents/registry.db" "SELECT COUNT(*) FROM agents" 2>/dev/null)
        if [[ -n "$agent_count" && "$agent_count" -gt 0 ]]; then
            validation_pass "Registry database valid ($agent_count agents)"
        else
            validation_error "Registry database empty or corrupt: $euxis_home/euxis-data/agents/registry.db"
            return 1
        fi
    elif [[ -f "$euxis_home/euxis-data/agents/registry.json" ]]; then
        # Fall back to JSON validation
        if ! jq empty "$euxis_home/euxis-data/agents/registry.json" &>/dev/null; then
            validation_error "Registry file is not valid JSON: $euxis_home/euxis-data/agents/registry.json"
            return 1
        fi
    else
        validation_error "Registry not found (neither registry.db nor euxis-data/agents/registry.json)"
        return 1
    fi
