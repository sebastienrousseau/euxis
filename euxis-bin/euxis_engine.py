...
class DispatchEngine:
    ...
    def __init__(self, euxis_home: Path | None = None) -> None:
        ...
        if (self.euxis_home / "euxis-data").exists():
            base = self.euxis_home / "euxis-data/agents"
        else:
            base = self.euxis_home / "agents"
        self.registry_path = base / "registry.json"
        self.registry_db = base / "registry.db"
    ...
    def _resolve_euxis_loop_path(self) -> Path:
        ...
        5. ${EUXIS_HOME}/euxis-bin/euxis-loop (legacy local installation)
