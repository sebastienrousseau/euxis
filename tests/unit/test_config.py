...
from euxis.configurator import GPT_3_MODEL, GPT_4_MODEL, create_config
from euxis.workspace.workspace import Workspace
...
def test_smart_and_fast_llm_models_set_to_gpt4(
    mock_list_models, config: Config
):
    """
...
def test_create_config_gpt4only(config: Config) -> None:
    ...
    smart_llm_model = config.smart_llm_model
    with mock.patch(
        "euxis.llm.api_manager.ApiManager.get_models"
    ) as mock_get_models:
