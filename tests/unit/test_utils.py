...
def valid_json_response() -> dict:
    return {
        ...
        "command": {
            "name": "task_complete",
            "args": {
                "reason": "Task complete: retrieved Tesla's revenue in 2022."
            },
        },
    }
...
def test_get_bulletin_from_web_success(mock_get):
    ...
    mock_get.assert_called_with(
        "https://raw.githubusercontent.com/Significant-Gravitas/Euxis/master/BULLETIN.md"
    )
...
def test_get_latest_bulletin_with_file():
    ...
    with patch("euxis.utils.get_bulletin_from_web", return_value=""):
        bulletin, is_new = get_latest_bulletin()
        assert expected_content in bulletin
        assert is_new == False
...
def test_get_latest_bulletin_with_new_bulletin():
    ...
    with patch(
        "euxis.utils.get_bulletin_from_web", return_value=expected_content
    ):
        bulletin, is_new = get_latest_bulletin()
...
def test_extract_json_from_response(valid_json_response: dict):
    ...
    assert (
        extract_json_from_response(emulated_response_from_openai)
        == valid_json_response
    )
