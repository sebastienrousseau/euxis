...
class TestCheckMaintenanceStatus(unittest.TestCase):
    ...
    def test_unmaintained_package(self, mock_parse, mock_get_info):
        ...
        mock_parse.return_value = [("ancient-package", "0.1.0")]
        mock_get_info.return_value = {
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_multiple_release_versions(self, mock_parse, mock_get_info):
        ...
        mock_get_info.return_value = {
            "releases": {
                "1.0.0": [{"upload_time": (now - timedelta(days=400)).strftime("%Y-%m-%dT%H:%M:%S")}],
                "2.0.0": [{"upload_time": (now - timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%S")}]
            }
        }
