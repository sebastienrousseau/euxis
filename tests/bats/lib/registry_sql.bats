...
@test "_registry_pool_init creates pool directory" {
    rm -rf "${EUXIS_HOME}/data/registry_pool"
    _registry_pool_init
    [[ -d "${EUXIS_HOME}/data/registry_pool" ]]
}

@test "_registry_pool_init cleans stale connections" {
    # Create a stale lock with dead PID
    local stale_lock="${EUXIS_HOME}/data/registry_pool/conn_0.lock"
    mkdir -p "${stale_lock}"
...
@test "_registry_release_connection releases lock" {
    ...
    if [[ "${conn_id}" != "-1" ]]; then
        local lockdir="${EUXIS_HOME}/data/registry_pool/conn_${conn_id}.lock"
        [[ -d "${lockdir}" ]]
