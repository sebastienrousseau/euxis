...
def get_base_vcr_config(request):
    record_mode = request.config.getoption(
...
def vcr_cassette_dir(request):
    ...
    return os.path.join("tests/Euxis-test-cassettes", test_name)
...
def patched_api_requestor(mocker: MockerFixture):
    ...
    original_validate_headers = (
        openai.api_requestor.APIRequestor._validate_headers
    )
    ...
    if PROXY:
        mocker.patch(
            "openai.api_requestor.APIRequestor.__init__", new=patched_init
        )
        mocker.patch.object(
