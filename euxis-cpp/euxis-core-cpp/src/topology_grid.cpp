...
auto TopologyGrid::view() -> mdspan_2d<double> {
    return mdspan_2d<double>(data_.data(), n_, n_);
}

auto TopologyGrid::view() const -> mdspan_2d<const double> {
    return mdspan_2d<const double>(data_.data(), n_, n_);
}
