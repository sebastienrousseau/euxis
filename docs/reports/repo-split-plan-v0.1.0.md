# Repository Split — Monolith to Modular (v0.1.0)

## Executive Summary
Split Euxis into a small set of purpose-built repos that mirror today’s module boundaries: core runtime, gateway+adapters, CLI tools, TUI, agents/prompts, security policy, memory/metrics data tooling, and docs. This structure preserves developer velocity by keeping tightly-coupled surfaces together (CLI + core libs, Gateway + adapters) while enabling independent releases for UI, gateway, and tooling.
...
- `gateway/server.py` imports `adapters.registry` and `gateway.utils`.
  - Evidence: `gateway/server.py:30-31`.
- Adapters depend on Gateway utilities:
  - `adapters/slack_adapter.py` imports `gateway.utils` for `persist_message` and `timestamp`.
  - `adapters/telegram_adapter.py` imports `gateway.utils` for `persist_message` and `timestamp`.

**TUI depends on crypto_lib and its own core:**
- `tui/core/services.py` imports `crypto_lib` encryption helpers.
  - Evidence: `tui/core/services.py:21-23`.
- TUI screens import `tui.core` and `tui.widgets` internally (self-contained).

**Metrics are self-contained but used by tooling:**
- `metrics/metrics_cli.py` imports `metrics/aggregators/performance_analyzer`.
  - Evidence: `metrics/metrics_cli.py:19`.
- Verification components import metrics analyzers.
  - Evidence: `metrics/verification/evidence_framework.py:27`.

**Security policy is read by Gateway:**
- Gateway config defaults load from `~/.euxis/security/gateway.json`.
  - Evidence: `gateway/server.py:133`.

### Circular Dependencies
No module-level circular imports were detected in Python packages (Gateway, Adapters, TUI, Metrics, crypto_lib). Shell scripts depend on `core/lib` but `core/lib` does not import from CLI. This yields a directed dependency graph with `core/` as the root.
...
### Data Flow (runtime)
- Gateway writes session, approvals, audit, transcripts to `~/.euxis/data/gateway/*` via `gateway/utils.py`.
- Cortex memory stored under `~/.euxis/memory/cortex` and accessed by `cli/bin/euxis-cortex` and `cli/bin/euxis-graph`.
...
- Gateway protocol tests live under `gateway/protocol_test.py`.
- TUI tests in `tests/unit/test_*` reference `tui/` screens.
...
5. **`euxis-tui`**
   ...
   - **Dependencies:** `euxis-core`, `crypto_lib`.
...
- HTTP endpoints: `gateway/server.py` (`/health`, `/approvals/*`).
...
### Data Ownership
- Gateway owns `~/.euxis/data/gateway/*`.
- Memory owns `~/.euxis/memory/cortex/*`.
...
4. **Extract `euxis-gateway` + `euxis-adapters`**
   - Move `gateway/` and `adapters/` into separate repos or a single repo with packages.

5. **Extract `euxis-tui`**
   - Move `tui/` and rewire `crypto_lib` dependency.

6. **Extract `euxis-metrics`**
   - Move `metrics/` and keep optional core schema references.

7. **Extract `euxis-docs`**
   - Move `docs/` and doc tooling config.

8. **Extract crypto packages**
   - Split `crypto_lib/` and `packages/*` into their dedicated repos.
...
1. Keep **Gateway + Adapters** together for initial split due to direct imports in `adapters/*` -> `gateway/utils.py`.
...
5. Split **crypto_lib** and **packages/** into dedicated repos due to different languages/toolchains.
