...
| `_perf_record` | `_perf_record "op" "$ms" "agent" "ok"` | Append JSONL metric to `$EUXIS_HOME/data/runtime/perf/metrics.jsonl` |
...
- `EUXIS_PERF_LOG` — Path to JSONL performance metrics file (default: `$EUXIS_HOME/data/runtime/perf/metrics.jsonl`)
...
| `euxis-bin/euxis-graph` | Knowledge graph operations (add-edge, query) |
| `euxis-bin/euxis-kaizen` | Automated maintenance (memory pruning, stale cleanup) |
