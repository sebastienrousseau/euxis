...
cat ~/.euxis/data/lifecycle/*.state

# List active agents
for f in ~/.euxis/data/lifecycle/*.state; do
...
tail -20 ~/.euxis/data/lifecycle/transitions.jsonl
```
...
2. **Force state reset:**
   ...
   echo "idle" > ~/.euxis/data/lifecycle/architect.state
...
mkdir -p ~/.euxis/data/lifecycle

# Check permissions
chmod 755 ~/.euxis/data/lifecycle

# Clear corrupted state files
rm ~/.euxis/data/lifecycle/*.state
```
...
cat ~/.euxis/data/projects/*/*/memory.md | head -20

# Look for binary or corrupted content
file ~/.euxis/data/projects/*/*/memory.md
```
...
1. **Reset memory for specific agent:**
   ...
   # Backup
   cp ~/.euxis/data/projects/$PROJECT/$AGENT/memory.md{,.bak}

   # Reset
   echo "# Memory: $AGENT" > ~/.euxis/data/projects/$PROJECT/$AGENT/memory.md
   ```

2. **Restore from backup:**
   ```bash
   cp ~/.euxis/data/projects/$PROJECT/$AGENT/memory.md.bak \
      ~/.euxis/data/projects/$PROJECT/$AGENT/memory.md
   ```
...
wc -l ~/.euxis/data/projects/*/*/memory.md
```
...
1. **Run manual pruning:**
   ...
   prune_project_memory ~/.euxis/data/projects/myproject
   ```
...
1. **Review and resolve contradictions:**
   ```bash
   # View drift warnings
   grep -r "\[DRIFT\]" ~/.euxis/data/projects/*/*/memory.md
   ```
...
2. **Manually resolve:**
   ...
   resolve_memory_contradiction \
       ~/.euxis/data/projects/myproject/architect/memory.md \
...
ls -la ~/.euxis/data/projects/myproject/

# Verify memory files exist for other agents
ls ~/.euxis/data/projects/myproject/*/memory.md
```
...
2. **Check directory structure:**
   ```bash
   # Memories should be at: ~/.euxis/data/projects/{project}/{agent}/memory.md
   mkdir -p ~/.euxis/data/projects/myproject/architect
   touch ~/.euxis/data/projects/myproject/architect/memory.md
   ```
...
| Bus messages | `~/.euxis/data/bus/pipes/*/*.msg` |
...
# Generate comprehensive system report
{
    ...
    echo "=== Recent Errors ==="
    grep -r "ERROR" ~/.euxis/data/lifecycle/transitions.jsonl 2>/dev/null | tail -20
    echo ""
