...
Comprehensive automation system ensuring Euxis v0.1.4 build/package/runtime parity across macOS, Linux, and WSL platforms.
...
**File:** `euxis-bin/euxis-cross-platform-verify`
...
### Gate 3: Core Scripts ✅
- Executable permission verification for all `euxis-bin/euxis*` scripts
- Script count validation (33+ scripts expected)
- Basic functionality test with timeout protection

### Gate 4: Agent Registry ✅
- JSON validity check for `data/agents/registry.json`
...
./euxis-bin/euxis-cross-platform-verify --local-only

# Test specific version
./euxis-bin/euxis-cross-platform-verify --version v0.1.4 --local-only
```

### Full Cross-Platform CI
```bash
# Trigger GitHub Actions workflow (requires gh CLI)
./euxis-bin/euxis-cross-platform-verify --version v0.1.4

# Manual GitHub workflow trigger
gh workflow run cross-platform-ci.yml --field version=v0.1.4
```

### Multi-Platform Container Build
```bash
# Build for AMD64 and ARM64
./deploy/build-multi-platform.sh --version v0.1.4
...
|---------------|--------|-----------|--------|
| v0.1.4        | 3.11+  | macOS, Linux, WSL | 20.10+ |
| v0.1.4+       | 3.11+  | macOS, Linux, WSL, ARM64 | 20.10+ |
...
**Verification Status for v0.1.4:** ✅ IMPLEMENTED
**Last Updated:**-02-01
**Next Review:** Release candidate testing

*Euxis v0.1.4 · Build something that matters.*