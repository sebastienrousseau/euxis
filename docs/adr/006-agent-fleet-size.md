...
1. **Consolidate to 10-15 agents** — Would reduce maintenance but lose domain specialization. The edge-hunter (security), rust-crate-steward (Rust ecosystem), and payments-domain-steward (PCI compliance) each carry specialized knowledge that a generic "security-agent" cannot replicate.
