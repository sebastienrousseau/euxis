...
if [[ ! -x "${TIMEOUT_BIN}" ]]; then
  echo "missing /usr/bin/timeout; cannot enforce bounded adapters test runtime" >&2
  exit 1
fi
...
done < <(find euxis-adapters/tests -maxdepth 2 -type f -name 'test_*.py' | sort)

if [[ ${status} -eq 0 ]]; then
  echo "== euxis-adapters/tests (coverage gate) =="
  pushd euxis-adapters >/dev/null
  PYTEST_LOCAL="${PYTEST}"
  ...
  if [[ ${gate_rc} -ne 0 ]]; then
    status=${gate_rc}
    echo "FAILED: euxis-adapters consolidated coverage gate (exit ${status})" >&2
  fi
fi
