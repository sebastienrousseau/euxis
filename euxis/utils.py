...
def clean_input(config: Config, prompt: str = "", talk=False):
    try:
        ...
        logger.info("You interrupted Euxis")
...
def get_latest_bulletin() -> tuple[str, bool]:
    ...
    news_header = Fore.YELLOW + "Welcome to Euxis!\n"
    if new_bulletin or current_bulletin:
        news_header += (
            "Below you'll find the latest Euxis News and updates regarding features!\n"
            "If you don't wish to see this message, you "
            "can run Euxis with the *--skip-news* flag.\n"
        )

    if new_bulletin and is_new_news:
        open("data/CURRENT_BULLETIN.md", "w", encoding="utf-8").write(
            new_bulletin
        )
        current_bulletin = (
            f"{Fore.RED}::NEW BULLETIN::{Fore.RESET}\n\n{new_bulletin}"
        )
