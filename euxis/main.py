...
from euxis.configurator import create_config
...
#from euxis.plugins import scan_plugins
from euxis.prompts.prompt import (
    DEFAULT_TRIGGERING_PROMPT,
    construct_main_ai_config,
)
from euxis.utils import (
...
COMMAND_CATEGORIES = [
    "euxis.commands.execute_code",
    "euxis.commands.file_operations",
    "euxis.commands.web_search",
    "euxis.commands.web_selenium",
    "euxis.app",
    "euxis.commands.task_statuses",
]
...
def run_auto_gpt(
    ...
    if not config.skip_news:
        ...
        if sys.version_info < (3, 10):
            logger.typewriter_log(
                ...
                "parts of Euxis with this version. "
    ...
    #   the env file.
    workspace_directory = Workspace.get_workspace_directory(
    ...
    if config.chat_messages_enabled:
        for plugin in config.plugins:
            if (
                hasattr(plugin, "can_handle_report")
                and plugin.can_handle_report()
            ):
                logger.info(
    ...
    logger.typewriter_log(
        "Using Browser:", Fore.GREEN, config.selenium_web_browser
    )
