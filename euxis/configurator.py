...
from euxis import utils
from euxis.llm.utils import check_model
from euxis.logs import logger
from euxis.memory.vector import get_supported_memory_backends
...
def create_config(
    ...
    Args:
        ...
        allow_downloads (bool): Whether to allow Euxis to download files natively
    ...
    if gpt3only:
        ...
        gpt4only
        and check_model(GPT_4_MODEL, model_type="smart_llm_model")
        == GPT_4_MODEL
    ):
        ...
        config.fast_llm_model = check_model(
            config.fast_llm_model, "fast_llm_model"
        )
        config.smart_llm_model = check_model(
    ...
    if allow_downloads:
        ...
        logger.typewriter_log(
            "WARNING: ",
            Fore.YELLOW,
            f"{Back.LIGHTYELLOW_EX}Euxis will now be able to download and save files to your machine.{Back.RESET} "
