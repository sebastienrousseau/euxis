...
from euxis.json_utils.utilities import (
    extract_json_from_response,
    validate_json,
)
from euxis.llm.chat import chat_with_ai
from euxis.llm.providers.openai import OPEN_AI_CHAT_MODELS
from euxis.llm.utils import count_string_tokens
from euxis.log_cycle.log_cycle import (
    FULL_MESSAGE_HISTORY_FILE_NAME,
    NEXT_ACTION_FILE_NAME,
    USER_INPUT_FILE_NAME,
    LogCycleHandler,
)
from euxis.logs import logger, print_assistant_thoughts, remove_ansi_escape
from euxis.memory.message_history import MessageHistory
from euxis.memory.vector import VectorMemory
from euxis.models.command_registry import CommandRegistry
from euxis.speech import say_text
from euxis.spinner import Spinner
from euxis.utils import clean_input
from euxis.workspace import Workspace


class Agent:
    """Agent class for interacting with Euxis.
    ...
    def __init__(
        ...
        self.workspace = Workspace(
            workspace_directory, config.restrict_to_workspace
        )
    ...
    def start_interaction_loop(self):
        ...
        from euxis.app import execute_command, get_command
        ...
        while True:
            ...
            with Spinner(
                "Thinking... ", plain_output=self.config.plain_output
            ):
            ...
            for plugin in self.config.plugins:
                if not plugin.can_handle_post_planning():
                    continue
                assistant_reply_json = plugin.post_planning(
                    assistant_reply_json
                )
            ...
            if not self.config.continuous_mode and self.next_action_count == 0:
                ...
                while True:
                    ...
                    if (
                        console_input.lower().strip()
                        == self.config.authorise_key
                    ):
            ...
            if command_name is not None and command_name.lower().startswith(
                ...
                result = (
                    f"Command {command_name} returned: " f"{command_result}"
                )
                ...
                memory_tlength = count_string_tokens(
                    str(self.history.summary_message()),
                    self.config.fast_llm_model,
                )
                if (
                    result_tlength + memory_tlength + 600
                    > self.fast_token_limit
                ):
