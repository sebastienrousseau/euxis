...
from euxis.llm.chat import create_chat_completion
from euxis.logs import logger
from euxis.prompts.default_prompts import (
...
def prompt_user(config: Config) -> AIConfig:
    ...
    user_desire = utils.clean_input(
        config, f"{Fore.LIGHTBLUE_EX}I want Euxis to{Style.RESET_ALL}: "
    )
...
def generate_aiconfig_automatic(user_prompt: str, config: Config) -> AIConfig:
    ...
    # Parse the output
    ai_name = re.search(
        r"Name(?:\s*):(?:\s*)(.*)", output, re.IGNORECASE
    ).group(1)
    ai_role = (
        re.search(
            r"Description(?:\s*):(?:\s*)(.*?)(?:(?:\n)|Goals)",
